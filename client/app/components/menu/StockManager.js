...
// or remove an item — all without leaving the tables screen. Adding brand-new
// items still lives on the full Menu page (linked via menuHref).
export default function StockManager({ isOpen, onClose, branchId, branchName, menuHref }) {
  const { user } = useAuth();
  ...
  const scoped = branchId && branchId !== 'All' && branchId !== 'all';
  ...
  const load = useCallback(async () => {
    if (!isOpen || !scoped) return;
    setLoading(true);
    try {
      const res = await api.get(`/menu?locationId=${branchId}&limit=500`);
      setItems(res.data?.data || []);
    } catch { toast.error('Could not load stock'); }
    finally { setLoading(false); }
  }, [isOpen, scoped, branchId]);
  ...
  const adjust = async (it, delta) => {
    ...
    try {
      const res = await api.patch(`/menu/${it._id}/stock/adjust`, { delta, branchId });
      const s = res.data?.data;
      if (s && typeof s.stock === 'number') patchItem(it._id, { branchSpecificStock: s.stock, isAvailable: s.isAvailable });
    } catch (e) {
      patchItem(it._id, { branchSpecificStock: cur }); // revert
  ...
  const setExact = async (it, value) => {
    ...
    setBusy(`${it._id}:set`);
    try {
      const res = await api.put(`/menu/${it._id}/stock`, { stock, branchId });
      const s = res.data?.data;
      if (s && typeof s.stock === 'number') patchItem(it._id, { branchSpecificStock: s.stock, isAvailable: s.isAvailable });
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Stock · ${branchName || 'Branch'}`} maxWidth="max-w-2xl">
      {!scoped ? (
        <div className="py-12 text-center text-sm text-(--color-text-muted)">
          <PackageOpen size={40} className="mx-auto mb-3 opacity-30" />
          Pick a specific branch first to manage its stock.
        </div>
      ) : (
