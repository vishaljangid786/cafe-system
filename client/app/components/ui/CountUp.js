...
export function CountUp({ value = 0, duration = 900, prefix = '', suffix = '', decimals = 0, locale = 'en-IN', format, className, title }) {
  ...
  if (typeof format === 'function') {
    return <span className={className} title={title}>{format(Number(display))}</span>;
  }
  ...
  return <span className={className} title={title}>{prefix}{formatted}{suffix}</span>;
}
