...
import UserPermissionEditor, { permissionList } from './UserPermissionEditor';
...
export default function PermissionManager({ className = "" }) {
  ...
  // PUT /users/:id/permissions is role-locked server-side to these roles with no
  // permission fallback. Mirror it here so we never offer an Edit button whose
  // save is guaranteed to 403 (e.g. a location_admin, or staff granted the page).
  ...
  return (
    <div className={`space-y-8 ${className}`}>
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-right">
                        {canEditPermissions ? (
                          ...
                          <span className="text-xs text-(--color-text-muted)">View only</span>
                        )}
                      </td>
                    </motion.tr>
                  );
      ...
      {/* Edit user permissions (shared editor — also used by the People page) */}
      <UserPermissionEditor
        isOpen={isModalOpen}
        onClose={() => setIsModalOpen(false)}
        targetUser={selectedUser}
        onSaved={fetchSubordinates}
        presets={presets}
      />
