  ...
  return (
    <PageTransition>
      <div className="space-y-10 pb-20 max-w-7xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
            ...
            <div className="flex items-center gap-6 bg-[var(--color-surface)]/40  p-6 rounded-xl border border-[var(--color-border)] shadow-sm">
              <div className="text-right">
                <p className="text-[10px] font-bold text-[var(--color-text-muted)] uppercase tracking-normal mb-1">Total Actions</p>
                <p className="text-2xl font-bold text-[var(--color-text-primary)]">{totalLogs}</p>
              </div>
              <div className="h-12 w-[1px] bg-[var(--color-border)]" />
              <div className="text-right">
                <p className="text-[10px] font-bold text-[var(--color-text-muted)] uppercase tracking-normal mb-1">Status</p>
                <p className="text-2xl font-bold text-[var(--color-success)]">ACTIVE</p>
              </div>
            </div>
          </div>
        </SlideIn>
        ...
        {/* Data List */}
        <div className="bg-[var(--color-surface)]/40  rounded-xl border border-[var(--color-border)] shadow-sm overflow-hidden">
                ...
                <AnimatePresence mode="popLayout">
                  {refetching ? (
                    ...
                    >
                      ...
                      <td className="px-10 py-6">
                        <div className="flex items-center gap-4">
                          <div className="h-10 w-10 rounded-xl bg-[var(--color-surface-soft)] flex items-center justify-center font-bold text-[10px] text-[var(--color-text-muted)] border border-[var(--color-border)]">
                      ...
                      <td className="px-10 py-6 text-right">
                        <p className="text-xs font-bold text-[var(--color-text-primary)] italic">{new Date(log.createdAt).toLocaleTimeString()}</p>
                        <p className="text-[9px] font-bold text-[var(--color-text-muted)] uppercase tracking-tight mt-1">{new Date(log.createdAt).toLocaleDateString()}</p>
                      </td>
                    </motion.tr>
                  ))}
                </AnimatePresence>
            ...
            <div className="px-10 py-8 bg-[var(--color-surface-soft)]/50 flex items-center justify-between border-t border-[var(--color-border)]">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  disabled={page === 1}
                  className="px-6 py-2.5 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl text-[10px] font-bold uppercase tracking-normal  active:scale-95 disabled:opacity-30 transition-all shadow-sm"
                >
                  Previous
                </button>
                <button
                  ...
                  className="px-6 py-2.5 bg-[var(--color-text-primary)] text-[var(--color-bg-base)] rounded-xl text-[10px] font-bold uppercase tracking-normal  active:scale-95 disabled:opacity-30 transition-all shadow-sm "
                >
                  Next
                </button>
              </div>
...
function SecurityCard({ title, value, icon, sub }) {
  return (
    <div className="bg-[var(--color-surface)]/40  p-8 rounded-xl border border-[var(--color-border)] shadow-sm group  transition-all duration-500">
      <div className="flex items-center justify-between mb-6">
        <div className="p-4 bg-[var(--color-surface-soft)] rounded-xl group-hover:rotate-12 transition-transform duration-500">
          {icon}
        </div>
        <div className="h-2 w-2 rounded-full bg-[var(--color-success)] " />
      </div>
      <p className="text-[10px] font-bold text-[var(--color-text-muted)] uppercase tracking-normal mb-1">{title}</p>
      <h3 className="text-4xl font-bold text-[var(--color-text-primary)] tracking-tight italic">{value}</h3>
