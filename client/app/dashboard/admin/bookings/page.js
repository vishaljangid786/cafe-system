    ...
    <PageTransition>
      <div className="space-y-8 pb-20">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          ...
          <div className="flex gap-3 bg-[var(--color-surface-soft)] p-1.5 rounded-xl border border-[var(--color-border)] shadow-sm">
            <button
              onClick={() => setViewMode('list')}
              className={`p-2.5 rounded-xl transition-all ${viewMode === 'list' ? 'bg-[var(--color-primary)] text-[var(--color-on-primary)] dark:text-[var(--color-on-primary)] shadow-lg ' : 'text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)] hover:bg-[var(--color-bg-soft)]'}`}
            >
              <List size={20} />
            </button>
            <button
              onClick={() => setViewMode('grid')}
              className={`p-2.5 rounded-xl transition-all ${viewMode === 'grid' ? 'bg-[var(--color-primary)] text-[var(--color-on-primary)] dark:text-[var(--color-on-primary)] shadow-lg ' : 'text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)] hover:bg-[var(--color-bg-soft)]'}`}
            >
              <LayoutGrid size={20} />
            </button>
          </div>
        ...
        <SlideIn direction="down">
          <div className="bg-[var(--color-surface)]/40 p-8 rounded-xl border border-[var(--color-border)] shadow-sm space-y-6  transition-colors">
            <div className="flex flex-col md:flex-row gap-4 items-center">
              <div className="flex gap-4 w-full md:w-auto">
                <div className="relative flex-1 md:w-64">
                  ...
                  <input
                    type="date"
                    className="w-full pl-12 pr-4 py-4 bg-[var(--color-bg-soft)] border border-[var(--color-border)] rounded-xl focus:ring-2 focus:ring-[var(--color-primary)] outline-none font-bold text-sm text-[var(--color-text-primary)] appearance-none"
                    value={dateFilter}
              ...
              <table className="w-full text-left border-collapse min-w-[800px]">
                <thead>
                  <tr className="bg-[var(--color-surface-soft)] border-b border-[var(--color-border)]">
                    <th className="px-8 py-5 text-[10px] font-bold uppercase tracking-normal text-[var(--color-text-muted)]">Customer</th>
                    <th className="px-8 py-5 text-[10px] font-bold uppercase tracking-normal text-[var(--color-text-muted)]">Time & Date</th>
                    <th className="px-8 py-5 text-[10px] font-bold uppercase tracking-normal text-[var(--color-text-muted)]">Guests</th>
                    ...
                    >
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-4">
                          <div className="h-10 w-10 rounded-xl bg-[var(--color-surface-soft)] flex items-center justify-center text-[var(--color-primary)] font-bold text-xs border border-[var(--color-border)]">
                            {booking.userId?.name?.substring(0, 2).toUpperCase()}
                          </div>
                          <div>
                            <p className="text-sm font-bold text-[var(--color-text-primary)] leading-none">{booking.userId?.name}</p>
                      ...
                      <td className="px-8 py-6">
                        <div className="space-y-1">
                          ...
                          <div className="text-[10px] text-[var(--color-text-muted)] font-bold uppercase tracking-normal">
                      ...
                      <td className="px-8 py-6 text-right">
                        <div className="flex justify-end gap-2">
                          {booking.status === 'pending' && (
                            <>
                              <button
                                ...
                                className="p-2.5 rounded-xl bg-[var(--color-success)]/10 hover:bg-[rgba(var(--color-success-rgb),0.12)] text-[var(--color-success)] hover:text-[var(--color-on-primary)] dark:hover:text-[var(--color-on-primary)] transition-all"
                              >
                                <Check size={16} />
                              </button>
                              <button
                                onClick={() => updateStatus(booking._id, 'cancelled')}
                                className="p-2.5 rounded-xl bg-[var(--color-danger)]/10 hover:bg-[rgba(var(--color-danger-rgb),0.12)] text-[var(--color-danger)] hover:text-white transition-all"
                              >
                                <X size={16} />
                              </button>
                            </>
                          ...
                          <button
                            ...
                            className="p-2.5 rounded-xl bg-[var(--color-bg-soft)] hover:bg-[var(--color-primary)] text-[var(--color-text-muted)] hover:text-[var(--color-on-primary)] dark:hover:text-[var(--color-on-primary)] transition-all border border-[var(--color-border)]"
                          >
                            <Zap size={16} />
                          </button>
                        </div>
                ...
                <CardHover>
                  <Card className="!p-8 group relative overflow-hidden border-[var(--color-border)] bg-[var(--color-surface)]/40 shadow-sm transition-colors">
                    ...
                    <div className="flex items-center gap-4 mb-8">
                      <div className="h-12 w-12 rounded-xl bg-[var(--color-primary)]/10 border border-[var(--color-primary)]/20 flex items-center justify-center text-[var(--color-primary)] font-bold">
                        {booking.userId?.name?.substring(0, 2).toUpperCase()}
                      </div>
                      <div>
                        <h3 className="text-lg font-bold text-[var(--color-text-primary)] tracking-tight leading-none">{booking.userId?.name}</h3>
                    ...
                    <div className="mt-8 pt-6 border-t border-[var(--color-border)] flex gap-2">
                      {booking.status === 'pending' ? (
                        <>
                          <button
                            onClick={() => updateStatus(booking._id, 'confirmed')}
                            className="flex-1 py-3 rounded-xl bg-[var(--color-success)]/10 hover:bg-[rgba(var(--color-success-rgb),0.12)] text-[var(--color-success)] hover:text-[var(--color-on-primary)] dark:hover:text-[var(--color-on-primary)] text-[10px] font-bold uppercase tracking-normal transition-all border border-[var(--color-success)]/20"
                          >
                            Approve
                          </button>
                          <button
                            onClick={() => updateStatus(booking._id, 'cancelled')}
                            className="flex-1 py-3 rounded-xl bg-[var(--color-danger)]/10 hover:bg-[rgba(var(--color-danger-rgb),0.12)] text-[var(--color-danger)] hover:text-white text-[10px] font-bold uppercase tracking-normal transition-all border border-[var(--color-danger)]/20"
                          >
                            Deny
                          </button>
                        </>
                        ...
                        <button
                          onClick={() => { setSelectedBooking(booking); setShowDetailModal(true); }}
                          className="w-full py-3 rounded-xl bg-[var(--color-surface-soft)] hover:bg-[var(--color-primary)] text-[var(--color-text-muted)] hover:text-[var(--color-on-primary)] dark:hover:text-[var(--color-on-primary)] text-[10px] font-bold uppercase tracking-normal transition-all"
                        >
                          View Details
                        </button>
                      )}
          ...
          {selectedBooking && (
            <div className="space-y-10">
              <div className="grid grid-cols-2 gap-10">
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-bold text-[var(--color-text-primary)]">{selectedBooking.userId?.name}</p>
                ...
                <div className="space-y-6">
                  <h4 className="text-[10px] font-bold uppercase tracking-normal text-[var(--color-text-muted)]">Booking Time</h4>
                  <div className="space-y-4">
                    <div className="flex items-center gap-3">
                      <Calendar size={18} className="text-[var(--color-primary)]" />
                      <p className="text-sm font-bold text-[var(--color-text-primary)]">{new Date(selectedBooking.date).toLocaleDateString(undefined, { dateStyle: 'full' })}</p>
                    </div>
                    <div className="flex items-center gap-3">
                      <Clock size={18} className="text-[var(--color-primary)]" />
                      <p className="text-sm font-bold text-[var(--color-text-primary)] tracking-normal">{selectedBooking.startTime} — {selectedBooking.endTime}</p>
                    </div>
                    <div className="flex items-center gap-3">
                      <Users size={18} className="text-[var(--color-primary)]" />
              ...
              <div className="p-6 bg-[var(--color-bg-soft)] border border-[var(--color-border)] rounded-xl space-y-3">
                <h4 className="text-[10px] font-bold uppercase tracking-normal text-[var(--color-primary)] flex items-center gap-2">
                ...
                {selectedBooking.status === 'pending' ? (
                  <>
                    <Button
                      variant="primary"
                      icon={Check}
                      className="flex-1 !py-5 shadow-sm  bg-[var(--color-success)] hover:bg-[var(--color-success-dark)] text-[var(--color-on-primary)] dark:text-[var(--color-on-primary)]"
                  ...
                  <Button
                    variant="primary"
                    icon={CheckCircle2}
                    className="w-full !py-5 bg-[var(--color-primary)] text-[var(--color-on-primary)] dark:text-[var(--color-on-primary)] hover:opacity-90"
