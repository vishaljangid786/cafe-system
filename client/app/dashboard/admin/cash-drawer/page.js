  ...
  return (
    <PageTransition>
      <div className="max-w-3xl mx-auto space-y-6">
        <SlideIn>
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-semibold text-(--color-text-primary) tracking-tight">Cash Drawer</h1>
                <p className="text-[11px] font-medium text-(--color-text-muted) tracking-normal">Open / close shift & reconcile cash</p>
              </div>
            </div>
            {!isBranchScoped && (
              <div className="w-48">
        ...
        {!session && (
          <SlideIn delay={0.05}>
            <div className="glass-card p-5 rounded-xl premium-shadow space-y-5">
              <div className="flex items-center gap-2 text-(--color-text-primary)">
                <LockOpen size={18} className="text-primary" />
                <h2 className="text-sm font-semibold">Open the drawer</h2>
              </div>
              ...
              <div className="flex gap-3 flex-wrap">
                <div className="relative flex-1 min-w-50">
                  ...
                  <input
                    ...
                    className="w-full pl-11 pr-4 py-3 rounded-xl bg-(--color-surface-soft) border border-(--color-border) text-sm font-medium text-(--color-text-primary) outline-none focus:border-primary"
                  />
                </div>
                {can(user, 'cashdrawer.add') && (
                  <button onClick={doOpen} disabled={busy} className="px-8 py-3 bg-primary text-(--color-on-primary) text-[11px] font-semibold uppercase tracking-normal rounded-xl hover:opacity-90 disabled:opacity-50">
                    Open drawer
                  </button>
                )}
              </div>
        ...
        {session && live && (
          <>
            <SlideIn delay={0.05}>
              <div className="glass-card p-5 rounded-xl premium-shadow space-y-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2 text-success">
                    <LockOpen size={18} />
                    <h2 className="text-sm font-semibold">Drawer open</h2>
                  </div>
                  <p className="text-[11px] font-medium text-(--color-text-muted) tracking-normal">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {[
                    ...
                    <div key={i} className={`p-4 rounded-xl border ${label === 'Expected in drawer' ? 'bg-primary/10 border-primary/30' : 'bg-(--color-surface-soft) border-(--color-border)'}`}>
                      <p className="text-[11px] font-medium tracking-normal text-(--color-text-muted)">{label}</p>
                      <p className={`text-lg font-semibold tracking-tight ${label === 'Expected in drawer' ? 'text-primary' : 'text-(--color-text-primary)'}`}>{money(val)}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {/* Pay in / out */}
            <SlideIn delay={0.1}>
              <div className="glass-card p-5 rounded-xl premium-shadow space-y-4">
                ...
                <div className="flex gap-3 flex-wrap">
                  <input type="number" value={moveAmount} onChange={(e) => setMoveAmount(e.target.value)} placeholder="Amount" min={0}
                    className="w-32 px-4 py-3 rounded-xl bg-(--color-surface-soft) border border-(--color-border) text-sm font-medium text-(--color-text-primary) outline-none focus:border-primary" />
                  ...
                  {can(user, 'cashdrawer.modify') && (
                    <button onClick={() => doMovement('in')} disabled={busy} className="flex items-center gap-1.5 px-4 py-3 bg-success/10 text-success text-[11px] font-semibold tracking-normal rounded-xl border border-success/20 hover:bg-success hover:text-white disabled:opacity-50">
                      <ArrowDownCircle size={14} /> Pay in
                    </button>
                  )}
                  {can(user, 'cashdrawer.modify') && (
                    <button onClick={() => doMovement('out')} disabled={busy} className="flex items-center gap-1.5 px-4 py-3 bg-danger/10 text-danger text-[11px] font-semibold tracking-normal rounded-xl border border-danger/20 hover:bg-danger hover:text-white disabled:opacity-50">
            ...
            {/* Close */}
            <SlideIn delay={0.15}>
              <div className="glass-card p-5 rounded-xl premium-shadow space-y-4 border border-danger/20">
                ...
                <div className="flex gap-3 flex-wrap">
                  ...
                  <input type="text" value={closeNotes} onChange={(e) => setCloseNotes(e.target.value)} placeholder="Notes (optional)"
                    className="flex-1 min-w-40 px-4 py-3 rounded-xl bg-(--color-surface-soft) border border-(--color-border) text-sm font-medium text-(--color-text-primary) outline-none focus:border-primary" />
                  {can(user, 'cashdrawer.modify') && (
                    <button onClick={doClose} disabled={busy} className="px-8 py-3 bg-danger text-white text-[11px] font-semibold uppercase tracking-normal rounded-xl hover:opacity-90 disabled:opacity-50">
                      Close & reconcile
                    </button>
                  )}
                </div>
          ...
          <SlideIn delay={0.2}>
            <div className="glass-card p-5 rounded-xl premium-shadow space-y-3">
              <div className="flex items-center gap-2 text-(--color-text-primary)">
                ...
                <h2 className="text-sm font-semibold">Past shifts (Z-reports)</h2>
              </div>
