      ...
      <PageTransition>
        <div className="p-8 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            {[1, 2, 3, 4].map(i => <div key={i} className="h-32 bg-[var(--color-surface-soft)] rounded-[2rem] animate-pulse" />)}
          </div>
        </div>
      </PageTransition>
    );
      ...
      <div className="p-6 lg:p-10 max-w-7xl mx-auto space-y-8">
        ...
        <SlideIn>
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <button 
              ...
              className="px-6 py-3 bg-[var(--color-text-primary)] text-[var(--color-bg-base)] text-xs font-black uppercase tracking-widest rounded-2xl hover:scale-105 transition-transform shadow-xl shadow-zinc-900/10"
            >
              Refresh Information
            </button>
          </div>
        ...
        {analytics && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <SlideIn delay={0.1}>
              <div className="bg-[var(--color-surface)]/40 backdrop-blur-2xl p-6 rounded-3xl border border-[var(--color-border)] shadow-sm relative overflow-hidden group">
                <div className="absolute -right-4 -bottom-4 opacity-5 group-hover:opacity-10 transition-opacity">
                  <Users size={120} />
                </div>
                <p className="text-[10px] font-black uppercase tracking-widest text-[var(--color-text-muted)]">Total Reach</p>
                <p className="text-4xl font-black text-[var(--color-text-primary)] mt-2">{analytics.totalCustomers}</p>
            ...
            <SlideIn delay={0.2}>
              <div className="bg-[var(--color-surface)]/40 backdrop-blur-2xl p-6 rounded-3xl border border-[var(--color-border)] shadow-sm relative overflow-hidden group">
                ...
                <p className="text-[10px] font-black uppercase tracking-widest text-[var(--color-text-muted)]">Repeat Rate</p>
                <p className="text-4xl font-black text-[var(--color-text-primary)] mt-2">{analytics.repeatRate}%</p>
                <p className="text-xs font-bold text-[var(--color-success)] mt-2 flex items-center gap-1">
            ...
            <SlideIn delay={0.3}>
              <div className="bg-[var(--color-primary)] p-6 rounded-3xl shadow-xl shadow-[var(--color-primary)]/20 text-black relative overflow-hidden group hover:scale-[1.02] transition-transform cursor-pointer">
            ...
            <SlideIn delay={0.4}>
              <div className="bg-[var(--color-danger)]/10 p-6 rounded-3xl border border-[var(--color-danger)]/20 relative overflow-hidden">
                <p className="text-[10px] font-black uppercase tracking-widest text-[var(--color-danger)]">At Risk (Inactive)</p>
                <p className="text-4xl font-black text-[var(--color-danger)] mt-2">{analytics.inactiveCustomersCount}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-[var(--color-border)] bg-[var(--color-surface-soft)]/50">
                      <th className="p-6 text-[10px] font-black uppercase tracking-widest text-[var(--color-text-muted)]">Rank</th>
                      <th className="p-6 text-[10px] font-black uppercase tracking-widest text-[var(--color-text-muted)]">Customer</th>
                      <th className="p-6 text-[10px] font-black uppercase tracking-widest text-[var(--color-text-muted)]">Tier</th>
                      <th className="p-6 text-[10px] font-black uppercase tracking-widest text-[var(--color-text-muted)]">Total Spend</th>
                      <th className="p-6 text-[10px] font-black uppercase tracking-widest text-[var(--color-text-muted)]">Visits</th>
                  ...
                  <tbody className="divide-y divide-[var(--color-border)]">
                    {topCustomers.map((cust, idx) => {
                      ...
                      return (
                        <tr key={cust._id} className="hover:bg-blue-500/[0.02] transition-colors group">
                          <td className="p-6">
                            <span className={`h-8 w-8 rounded-xl flex items-center justify-center text-xs font-black ${idx < 3 ? 'bg-[var(--color-primary)]/20 text-[var(--color-primary-dark)] dark:text-[var(--color-primary)]' : 'bg-[var(--color-surface-soft)] text-[var(--color-text-muted)]'}`}>
                              #{idx + 1}
                            </span>
                          </td>
                          <td className="p-6">
                          ...
                          <td className="p-6">
                            <p className="text-lg font-black text-[var(--color-text-primary)]">₹{cust.totalSpend?.toLocaleString()}</p>
                          </td>
                          <td className="p-6">
                          ...
                          <td className="p-6">
                            <button 
                              ...
                              className="p-2 rounded-xl bg-[var(--color-surface-soft)] text-[var(--color-text-muted)] hover:bg-[var(--color-primary)] hover:text-black transition-all shadow-sm opacity-0 group-hover:opacity-100"
                            >
          ...
          <div className="space-y-6">
            ...
            <div className="bg-[var(--color-surface)] rounded-[2.5rem] border border-[var(--color-border)] p-6 shadow-sm flex flex-col gap-4">
              {inactiveCustomers.map(cust => (
                <div key={cust._id} className="p-4 rounded-2xl bg-[var(--color-surface-soft)] border border-[var(--color-border)] flex justify-between items-center group hover:border-rose-500/30 transition-colors">
                  <div>
                    <p className="font-bold text-[var(--color-text-primary)]">{cust.name}</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          ...
          >
            <div className="p-8 border-b border-[var(--color-border)] flex justify-between items-start">
              <div>
                <h3 className="text-xl font-black text-[var(--color-text-primary)]">Customer <span className="text-[var(--color-primary)]">Profile</span></h3>
                <p className="text-[10px] font-black uppercase tracking-widest text-[var(--color-text-muted)] mt-1">ID: {viewingCustomer._id}</p>
              </div>
            ...
            <div className="p-8 space-y-6">
              <div className="flex items-center gap-4 p-4 bg-[var(--color-surface-soft)] rounded-2xl border border-[var(--color-border)]">
                <div className="h-14 w-14 rounded-2xl bg-[var(--color-primary)] text-black flex items-center justify-center text-2xl font-black shadow-lg shadow-[var(--color-primary)]/20">
                  {viewingCustomer.name?.charAt(0) || 'C'}
                </div>
                <div>
                  <p className="font-black text-xl text-[var(--color-text-primary)]">{viewingCustomer.name}</p>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 rounded-2xl bg-[var(--color-success)]/10 border border-[var(--color-success)]/20">
                  <p className="text-[10px] font-black uppercase tracking-widest text-[var(--color-success)] mb-1">Lifetime Value</p>
            ...
            <div className="p-8 bg-[var(--color-surface-soft)]/30 border-t border-[var(--color-border)]">
              <button
                ...
                className="w-full py-4 rounded-2xl bg-[var(--color-text-primary)] text-[var(--color-bg-base)] text-xs font-black uppercase tracking-widest transition-all hover:scale-[1.02] shadow-xl shadow-zinc-900/10"
              >
                Close Profile
              </button>
            </div>
