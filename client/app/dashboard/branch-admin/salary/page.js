    ...
    <PageTransition>
      <div className="space-y-6 pb-10">
        ...
        <SlideIn direction="down">
          <div className="relative overflow-hidden bg-(--color-surface) dark:bg-(--color-surface) p-5 rounded-xl shadow-sm border border-(--color-border) dark:border-(--color-border)">
            ...
            <div className="relative z-10 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-5">
              <div>
                <div className="flex items-center space-x-3 mb-4">
                  <div className="px-3 py-1 bg-(--color-primary-soft) text-primary rounded-full text-[11px] font-medium border border-[rgba(var(--color-primary-rgb),0.2)]">
                    Salary
                  </div>
                </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto">
                ...
                >
                  ...
                  <input
                    ref={monthInputRef}
                    type="month"
                    className="bg-transparent border-none outline-none py-2 text-sm font-medium text-(--color-text-primary) dark:text-(--color-text-primary) cursor-pointer"
                    value={month}
                ...
                <button
                  ...
                  className="px-5 py-4 bg-(--color-surface-soft) text-(--color-text-primary) border border-(--color-border) rounded-xl font-medium text-xs  transition-all shadow-sm flex items-center justify-center"
                >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <SlideIn delay={0.1}>
            <div className="bg-primary p-5 rounded-xl text-white shadow-sm  relative overflow-hidden group">
              ...
              <div className="mt-6 flex items-center text-[11px] font-medium bg-white/10 w-fit px-2.5 py-1 rounded-full border border-(--color-border)">
                    ...
                    >
                      <td className="px-5 py-4">
                        <div className="flex items-center">
                          ...
                          <div className="ml-6">
                            <div className="text-lg font-medium text-(--color-text-primary) dark:text-(--color-text-primary) tracking-tight group-hover:text-primary transition-colors">{item.name}</div>
                            <div className="text-[11px] font-medium text-(--color-text-muted) mt-1">{item.email}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-5 py-4">
                        <div className="flex flex-col items-center">
                          <div className="flex -space-x-2 mb-3">
                            <div className="w-10 h-10 rounded-xl bg-success text-white flex flex-col items-center justify-center border-2 border-(--color-border) dark:border-(--color-border) shadow-sm">
                              ...
                              <span className="text-[6px] font-medium uppercase">Prs</span>
                            </div>
                            <div className="w-10 h-10 rounded-xl bg-primary text-white flex flex-col items-center justify-center border-2 border-(--color-border) dark:border-(--color-border) shadow-sm">
                              ...
                              <span className="text-[6px] font-medium uppercase">Hlf</span>
                            </div>
                          ...
                          <div className="px-2.5 py-1 bg-(--color-surface-soft) dark:bg-(--color-surface) rounded-lg text-[11px] font-medium text-(--color-text-muted) dark:text-(--color-text-muted)">
          ...
          {viewingUser && (
            <div className="space-y-6">
              <div className="flex flex-col md:flex-row justify-between gap-6 border-b border-(--color-border) dark:border-(--color-border) pb-8">
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-medium text-(--color-text-muted) mt-2">{viewingUser.email}</p>
                ...
                <div className="text-right">
                  <p className="text-[11px] font-medium text-(--color-text-muted) uppercase tracking-normal mb-1">Base Salary</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="space-y-6">
                  <div>
                    <h4 className="text-[11px] font-medium uppercase tracking-normal text-(--color-text-muted) mb-4">Attendance ({month})</h4>
                ...
                <div className="space-y-6">
                  <div className="bg-(--color-surface) dark:bg-(--color-surface-soft) p-5 rounded-xl text-(--color-text-primary) shadow-sm relative overflow-hidden group">
                    ...
                    <p className="text-[11px] font-medium uppercase tracking-normal opacity-60 mb-4">Net Salary Payable</p>
