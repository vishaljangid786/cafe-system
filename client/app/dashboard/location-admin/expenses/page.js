    ...
    <PageTransition>
      <div className="space-y-8 pb-20">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-(--color-surface) dark:bg-(--color-surface) p-8 rounded-xl shadow-sm border border-(--color-border) dark:border-(--color-border) gap-6">
          <div>
            <h1 className="text-3xl font-bold text-(--color-text-primary) dark:text-(--color-text-primary) flex items-center tracking-tight leading-none">
              <TrendingDown className="mr-4 text-danger" size={36} /> Branch <span className="ml-3 text-danger">Expenses</span>
            </h1>
            ...
            <div className="flex items-center gap-3 bg-(--color-surface-soft) dark:bg-(--color-bg) p-1.5 rounded-xl border border-(--color-border) dark:border-(--color-border) shadow-inner">
              {['7d', '1m', 'all'].map(t => (
                <button
                  ...
                  className={`px-5 py-2 text-[10px] font-bold uppercase tracking-normal rounded-xl transition-all ${timeRange === t ? 'bg-danger text-white shadow-lg ' : 'text-(--color-text-muted) hover:text-(--color-text-primary) dark:hover:text-(--color-text-muted)'}`}
                >
                  {t}
                </button>
              ))}
        ...
        <SlideIn delay={0.1}>
          <div className="bg-white/40 dark:bg-(--color-bg)/20  p-8 rounded-xl border border-(--color-border) dark:border-(--color-border) shadow-sm relative overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-10 relative z-10">
              <h2 className="text-xl font-bold text-(--color-text-primary) dark:text-(--color-text-primary) tracking-tight">Expense Trend</h2>
              <div className="text-right">
                <p className="text-[10px] font-bold uppercase tracking-normal text-(--color-text-muted)">Total Spend</p>
                <p className="text-2xl font-bold text-danger tracking-tight">₹{totalExpenditure.toLocaleString()}</p>
              </div>
            </div>
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center">
          <div className="relative flex-1 w-full">
            ...
            <input 
              ...
              className="w-full pl-12 pr-4 py-4 bg-white/40 dark:bg-(--color-surface) border border-(--color-border) dark:border-(--color-border) rounded-xl focus:ring-2 focus:ring-danger/20 outline-none transition-all font-bold text-sm text-(--color-text-primary) dark:text-(--color-text-primary) shadow-sm"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
            />
          </div>
          <div className="flex gap-3 w-full md:w-auto">
            <Button variant="secondary" className="!py-4 px-6 rounded-xl border-none bg-danger/10 text-danger hover:bg-danger hover:text-white transition-all shadow-sm">
        ...
        <div className="space-y-4">
          <h3 className="text-[10px] font-bold uppercase tracking-normal text-(--color-text-muted) ml-1">Recent Expenses</h3>
          {refetching ? (
            <ListSkeleton rows={6} />
            ...
            <div className="py-20 text-center bg-white/40 dark:bg-(--color-bg)/40 rounded-xl border border-dashed border-(--color-border) dark:border-(--color-border)">
              <p className="text-(--color-text-muted) font-bold">No expenses found.</p>
            </div>
          ) : (
                    ...
                    <div className="bg-white/60 dark:bg-(--color-surface)/40  p-5 rounded-xl border border-(--color-border) dark:border-(--color-border) flex items-center justify-between group hover:border-danger/30 transition-all cursor-pointer">
                      <div className="flex items-center gap-5">
                        ...
                        <div>
                          ...
                          <p className="text-[8px] font-bold uppercase tracking-normal text-danger bg-danger/5 px-2 py-0.5 rounded-md mt-2 w-fit">
                            {t.category || 'Daily'}
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-8 text-right">
                        <div>
                          <p className="text-xl font-bold text-danger tracking-tight">-₹{t.totalAmount.toLocaleString()}</p>
                          <p className="text-[8px] font-bold uppercase tracking-normal text-(--color-text-muted) mt-1">Settled</p>
                        </div>
          ...
          <form onSubmit={handleAddExpense} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-1">
                <label className="text-[10px] font-bold uppercase tracking-normal text-(--color-text-muted) ml-1">Expense Title</label>
                <input required className="w-full rounded-xl bg-(--color-surface-soft) dark:bg-(--color-bg) border border-(--color-border) dark:border-(--color-border) p-4 text-sm font-bold dark:text-white" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} placeholder="e.g. Groceries" />
              </div>
              <div className="space-y-1">
                <label className="text-[10px] font-bold uppercase tracking-normal text-(--color-text-muted) ml-1">Amount (₹)</label>
                <input required type="number" className="w-full rounded-xl bg-(--color-surface-soft) dark:bg-(--color-bg) border border-(--color-border) dark:border-(--color-border) p-4 text-sm font-bold dark:text-white" value={formData.amount} onChange={e => setFormData({...formData, amount: e.target.value})} />
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <PremiumSelect 
              ...
              <div className="space-y-1">
                ...
                <input required type="date" className="w-full rounded-xl bg-(--color-surface-soft) dark:bg-(--color-bg) border border-(--color-border) dark:border-(--color-border) p-4 text-sm font-bold dark:text-white" value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} />
              </div>
            </div>
            <div className="space-y-1">
