      ...
      <div className="space-y-8 pb-20">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-[var(--color-surface)] dark:bg-[var(--color-surface)] p-8 rounded-xl shadow-sm border border-[var(--color-border)] dark:border-[var(--color-border)] gap-6">
          <div>
            ...
            <p className="text-[var(--color-text-muted)] dark:text-[var(--color-text-muted)] text-sm mt-2 font-medium">Track your branch income and daily sales.</p>
          </div>
          <div className="flex items-center gap-3 bg-[var(--color-surface-soft)] dark:bg-[var(--color-bg)] p-1.5 rounded-xl border border-[var(--color-border)] dark:border-[var(--color-border)] shadow-inner">
            {['7d', '1m', 'all'].map(t => (
              <button
                ...
                className={`px-5 py-2 text-[10px] font-bold uppercase tracking-normal rounded-xl transition-all ${timeRange === t ? 'bg-[var(--color-success)] text-white shadow-lg ' : 'text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)] dark:hover:text-[var(--color-text-muted)]'}`}
              >
                {t}
              </button>
            ))}
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center">
          ...
          <div className="flex gap-3 w-full md:w-auto">
            <Button variant="secondary" className="!py-4 px-6 rounded-xl border-none bg-[var(--color-success)]/10 text-[var(--color-success)] hover:bg-[var(--color-success)] hover:text-white transition-all shadow-sm">
        ...
        <div className="space-y-4">
          <h3 className="text-[10px] font-bold uppercase tracking-normal text-[var(--color-text-muted)] ml-1">Recent Income</h3>
          {refetching ? (
            <div className="space-y-4">
            ...
            <div className="py-20 text-center bg-white/40 dark:bg-[var(--color-bg)]/40 rounded-xl border border-dashed border-[var(--color-border)] dark:border-[var(--color-border)]">
              <p className="text-[var(--color-text-muted)] font-bold">No revenue found.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-3">
              {paginatedData.map((t, idx) => (
                <SlideIn key={t._id} delay={idx * 0.02}>
                  <CardHover>
                    <div className="bg-white/60 dark:bg-[var(--color-surface)]/40  p-5 rounded-xl border border-[var(--color-border)] dark:border-[var(--color-border)] flex items-center justify-between group hover:border-[var(--color-success)]/30 transition-all cursor-pointer">
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-center gap-3 pt-6">
            <button 
              ...
              className="h-10 w-10 rounded-xl bg-[var(--color-surface)] dark:bg-[var(--color-surface)] border border-[var(--color-border)] dark:border-[var(--color-border)] flex items-center justify-center text-[var(--color-text-muted)] disabled:opacity-30 transition-all hover:border-[var(--color-success)]/30"
            >
