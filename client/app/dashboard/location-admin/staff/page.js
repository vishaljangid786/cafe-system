            ...
            <div className="flex items-center space-x-6">
              ...
              <Link href="/dashboard/add-member">
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  className="bg-primary text-(--color-on-primary) px-5 py-3 rounded-xl font-semibold text-xs uppercase tracking-normal shadow-sm  flex items-center"
                >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            <SlideIn key={member._id} delay={i * 0.05}>
              <CardHover>
                <div
                  ...
                  className="bg-(--color-surface) dark:bg-(--color-surface) rounded-xl shadow-sm border border-(--color-border) dark:border-(--color-border) p-5 relative group overflow-hidden h-full flex flex-col cursor-pointer"
                >
                  ...
                  <div className="flex items-center space-x-5 relative z-10">
                    <div className="h-16 w-16 rounded-xl bg-(--color-primary-soft) flex items-center justify-center text-primary dark:text-primary border border-primary/20 shadow-inner">
                      <span className="text-2xl font-semibold uppercase">{member.name.charAt(0)}</span>
                    </div>
                    <div>
                      <h3 className="font-semibold text-(--color-text-primary) dark:text-(--color-text-primary) text-xl tracking-tight leading-tight">{member.name}</h3>
                      <div className="flex items-center mt-1">
                        <UserCheck size={12} className="text-primary mr-2" />
                        <span className="text-[11px] font-medium text-primary dark:text-primary uppercase tracking-normal">{member.role}</span>
                      </div>
                    </div>
                  </div>

                  <div className="mt-6 space-y-4 relative z-10 flex-grow">
                    <div className="flex items-center text-xs font-medium text-(--color-text-muted) dark:text-(--color-text-muted) group-hover:text-primary transition-colors">
                      <Mail size={16} className="mr-4 opacity-40" /> {member.email}
                    </div>
                    <div className="flex items-center text-xs font-medium text-(--color-text-muted) dark:text-(--color-text-muted)">
                      <Phone size={16} className="mr-4 opacity-40" /> {member.phone}
                    </div>
                    <div className="flex items-center text-xs font-medium text-(--color-text-muted) dark:text-(--color-text-muted) truncate">
                      <MapPin size={16} className="mr-4 opacity-40" /> {member.city}, {member.state}
                    </div>
                  </div>

                  <div className="mt-6 pt-6 border-t border-(--color-border) dark:border-(--color-border) flex justify-end items-center relative z-10">
          ...
          {staff.length === 0 && (
            <div className="col-span-full py-32 bg-(--color-surface) dark:bg-(--color-surface) rounded-xl border-4 border-dashed border-(--color-border) dark:border-(--color-border) flex flex-col items-center justify-center opacity-30">
              ...
              <p className="font-medium text-sm uppercase tracking-normal">Staff list is empty</p>
            </div>
          )}
        </div>
        ...
        >
          <form onSubmit={handleUpdate} className="space-y-6">
            <div className="grid grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-[11px] font-medium text-(--color-text-muted) tracking-normal mb-2 ml-1">Email Address</label>
                <input required type="email" className="w-full px-5 py-4 rounded-xl bg-(--color-surface-soft) dark:bg-(--color-surface)/50 border-none focus:ring-2 focus:ring-primary transition-all text-sm font-medium dark:text-(--color-text-primary) outline-none" value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
              <div>
                ...
                <input required className="w-full px-5 py-4 rounded-xl bg-(--color-surface-soft) dark:bg-(--color-surface)/50 border-none focus:ring-2 focus:ring-primary transition-all text-sm font-medium dark:text-(--color-text-primary) outline-none" value={formData.phone} onChange={e => setFormData({ ...formData, phone: e.target.value })} />
              </div>
              <div>
                <label className="block text-[11px] font-medium text-(--color-text-muted) tracking-normal mb-2 ml-1">Age</label>
                <input required type="number" className="w-full px-5 py-4 rounded-xl bg-(--color-surface-soft) dark:bg-(--color-surface)/50 border-none focus:ring-2 focus:ring-primary transition-all text-sm font-medium dark:text-(--color-text-primary) outline-none" value={formData.age} onChange={e => setFormData({ ...formData, age: e.target.value })} />
              </div>
                <PremiumSelect 
                  label="Gender"
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-6">
              ...
              <div>
                ...
                <input required className="w-full px-5 py-4 rounded-xl bg-(--color-surface-soft) dark:bg-(--color-surface)/50 border-none focus:ring-2 focus:ring-primary transition-all text-sm font-medium dark:text-(--color-text-primary) outline-none" value={formData.pincode} onChange={e => setFormData({ ...formData, pincode: e.target.value })} />
              </div>
              <div>
                <label className="block text-[11px] font-medium text-(--color-text-muted) tracking-normal mb-2 ml-1">Monthly Salary (₹)</label>
                <input required type="number" className="w-full px-5 py-4 rounded-xl bg-(--color-surface-soft) dark:bg-(--color-surface)/50 border-none focus:ring-2 focus:ring-primary transition-all text-sm font-medium dark:text-(--color-text-primary) outline-none" value={formData.monthlySalary} onChange={e => setFormData({ ...formData, monthlySalary: e.target.value })} />
              </div>
            </div>
                    ...
                    <div className="grid grid-cols-1 gap-6">
                      <div className="flex items-center gap-4 bg-(--color-surface-soft) dark:bg-(--color-surface)/50 p-4 rounded-xl border border-(--color-border) dark:border-(--color-border)">
                        ...
                        <div>
                          ...
                          <p className="text-sm font-medium text-(--color-text-secondary) dark:text-(--color-text-muted)">{viewingStaff.aadharNumber || 'Not provided'}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-4 bg-(--color-surface-soft) dark:bg-(--color-surface)/50 p-4 rounded-xl border border-(--color-border) dark:border-(--color-border)">
                        <Phone className="text-primary" size={20} />
                        <div>
                          <p className="text-[11px] font-medium uppercase text-(--color-text-muted) tracking-normal">Phone Number</p>
                    ...
                    {viewingStaff.aadharImage ? (
                      <div className="group relative rounded-xl overflow-hidden border border-(--color-border) dark:border-(--color-border) bg-(--color-surface-soft) dark:bg-(--color-surface) aspect-video">
                        ...
                        >
                          ...
                          <span className="font-medium text-[11px] uppercase tracking-normal">View Original Image</span>
                        </a>
                      </div>
                    ) : (
                      <div className="rounded-xl border-2 border-dashed border-(--color-border) dark:border-(--color-border) p-10 flex flex-col items-center justify-center text-(--color-text-muted) aspect-video">
                        ...
                        <p className="text-[11px] font-medium uppercase tracking-normal text-center">Aadhar Image Missing</p>
                      </div>
                    )}
                  </div>
                </div>
