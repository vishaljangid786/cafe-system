  ...
  return (
    <PageTransition>
      <div className="space-y-10 pb-20">
        ...
        <SlideIn delay={0.1}>
          <div className="relative overflow-hidden rounded-xl bg-(--color-surface-dark) p-10 lg:p-14 text-white shadow-sm shadow-black/30 border border-(--color-border)">
            {/* Background Effects */}
            ...
            <div className="relative z-10 flex flex-col lg:flex-row justify-between items-center gap-12">
              <div className="flex-1 space-y-8">
                <div>
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-normal text-primary/80">Salary for this month</span>
                  </div>
                  <h2 className="text-6xl font-bold tracking-tight mb-2">
                    ₹{Math.round(salaryData?.calculatedSalary || 0).toLocaleString()}
                  </h2>
                  <p className="text-(--color-text-muted) font-medium text-sm">Estimated payout for <span className="text-white font-bold">{new Date(month).toLocaleDateString(undefined, { month: 'long', year: 'numeric' })}</span>.</p>
                </div>

                <div className="flex flex-wrap gap-4">
                  <div className="px-5 py-3 rounded-xl bg-white/5 border border-(--color-border) ">
                    <p className="text-[8px] font-bold uppercase tracking-normal text-(--color-text-muted) mb-1">Basic Salary</p>
                    <p className="text-sm font-bold tracking-tight">₹{user.monthlySalary?.toLocaleString()}</p>
                  </div>
                  <div className="px-5 py-3 rounded-xl bg-white/5 border border-(--color-border) ">
                    <p className="text-[8px] font-bold uppercase tracking-normal text-(--color-text-muted) mb-1">Paid Days</p>
                    <p className="text-sm font-bold tracking-tight text-primary">{salaryData?.payableDays} Days</p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3 w-full lg:w-auto">
                {[
                  ...
                  <div key={stat.label} className="bg-white/5 p-6 rounded-xl border border-(--color-border) flex flex-col items-center text-center group hover:bg-white/10 transition-all duration-500">
                    ...
                    <p className="text-[8px] font-bold uppercase tracking-normal text-(--color-text-muted)">{stat.label}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-4 items-center px-4">
              <div className="relative flex-1 w-full">
                ...
                <input 
                  ...
                  className="w-full pl-12 pr-6 py-4 bg-(--color-surface)/40 border border-(--color-border) rounded-xl focus:ring-2 focus:ring-primary/20 outline-none transition-all font-bold text-sm text-(--color-text-primary) shadow-sm"
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                />
              </div>
              <div className="flex items-center gap-3">
                <div className="h-12 w-12 rounded-xl bg-(--color-surface-soft) border border-(--color-border) flex items-center justify-center text-(--color-text-muted)">
                  <Filter size={18} />
                </div>
                <div className="px-5 py-3.5 bg-(--color-surface-soft) text-(--color-text-primary) border border-(--color-border) rounded-xl text-[10px] font-bold uppercase tracking-normal shadow-lg">
                ...
                <table className="w-full text-left border-separate border-spacing-0">
                  <thead>
                    <tr className="bg-(--color-surface-soft)/50 text-[10px] font-bold uppercase tracking-normal text-(--color-text-muted)">
                      <th className="px-8 py-5 border-b border-(--color-border)">Date</th>
                      <th className="px-8 py-5 border-b border-(--color-border) text-center">Status</th>
                      <th className="px-8 py-5 border-b border-(--color-border) text-right">Time</th>
                    </tr>
                  </thead>
                      ...
                      <tr key={idx} className="group hover:bg-primary/[0.02] transition-colors duration-300">
                        <td className="px-8 py-5">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              ...
                              <p className="text-[8px] font-bold uppercase tracking-normal text-(--color-text-muted) mt-0.5">Marked</p>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-8 py-5 text-right">
                          <div className="flex flex-col items-end">
                            <p className="text-xs font-bold text-(--color-text-primary) tracking-tight">
                              {new Date(log.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true })}
                            </p>
                            <p className="text-[8px] font-bold text-(--color-text-muted) uppercase tracking-tight mt-1 flex items-center gap-1.5">
