...
const ACTION_SCOPES = [
  ...
  },
  {
    ...
    actions: [
      ...
      { action: 'delete', label: 'Delete ingredients / waste logs', legacy: { roles: ['admin'], perms: [] } },
    ],
  },
  {
    ...
    actions: [
      ...
      { action: 'delete', label: 'Delete purchase orders / suppliers', legacy: { roles: ['admin', 'branch_admin', 'location_admin'], perms: [] } },
    ],
  },
  {
    ...
    actions: [
      ...
      // A closed shift is the Z-report for that day, so removing one is an
      // admin-level act even though opening/closing is ordinary staff work.
      { action: 'delete', label: 'Delete a drawer shift', legacy: { roles: ['admin'], perms: [] } },
    ],
  },
  {
    ...
    actions: [
      ...
      { action: 'delete', label: 'Delete waitlist entries', legacy: { roles: [], perms: ['manageOrders'] } },
    ],
  },
  {
  ...
  },
  {
    ...
    actions: [
      ...
      // An unspent balance is a liability the customer owns — voiding it is an
      // admin decision, not a counter-staff one.
      { action: 'delete', label: 'Void gift cards', legacy: { roles: ['admin'], perms: [] } },
    ],
  },
  {
    // Customer-submitted ratings. page_feedback grants READ; this is the write.
    scope: 'feedback', pageKey: 'page_feedback', label: 'Feedback',
    actions: [
      { action: 'delete', label: 'Delete feedback entries', legacy: { roles: ['admin', 'branch_admin', 'location_admin'], perms: [] } },
    ],
  },
  ...
  // ---- Revenue ----------------------------------------------------------------
  {
    ...
    actions: [
      ...
      // Mirrors expenses.delete so money-in and money-out are governed the same
      // way. The controller adds the second layer: an APPROVED entry, or one
      // generated by a real order, is super_admin-only — see transactionController.
      { action: 'delete', label: 'Delete revenue entries', legacy: { roles: ['admin', 'branch_admin'], perms: ['editRevenue'] } },
    ],
  },
  {
  ...
  // action scope here — grant "Manage Staff" in the Permissions section instead.
  {
    ...
    actions: [
      ...
      // Attendance drives payroll, so removing a mark must be as deliberate as
      // marking one. Same legacy grant as 'add' — whoever can mark can unmark.
      { action: 'delete', label: 'Delete attendance records', legacy: { roles: [], perms: ['manageStaff'] } },
    ],
  },
  {
    ...
    actions: [
      ...
      // A PAID payroll row is proof someone was paid; the controller keeps that
      // one super_admin-only regardless of this flag.
      { action: 'delete', label: 'Delete payroll records', legacy: { roles: ['admin'], perms: [] } },
    ],
  },
